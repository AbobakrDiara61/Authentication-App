...
const signup = async (req, res) => {
    try {
        ...
        const refreshToken = createRefreshToken({ name, email, _id: user._id });
        setCookie(res, refreshToken, 'refreshToken');
        await sendVerificationEmail(email, verficationCode);
        return res.status(201).json({ message: "User created successfully", user: { ...user._doc, password: undefined } });
...
const login = async (req, res) => {
    try {
        ...
        try {
            ...
            setCookie(res, refreshToken, 'refreshToken');
            return res.status(200).json({ message: "Login successful", user: { ...user._doc, password: undefined } });
        } catch (error) {
...
const forgotPassword = async (req, res) => {
    try {
        ...
        return res.status(200).json({ message: "Reset Password Request Sent Successfully", user: {
            ...user._doc,
            password: undefined
        }});
